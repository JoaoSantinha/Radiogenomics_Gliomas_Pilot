...
n_repeats <- 10
n_folds <- 3
highlyCorrelatedCutoff <- 1.1 # do not use highly correlated cut-off
n_lambdas <- 100
min_lambda <- -10
max_lambda <- 3
positive_class <- "WT"
...
preprocessParams <- caret::preProcess(radiomics_df_hgg_lgg_sorted_clinical_noID, method=c("zv", "nzv", "center", "scale"), na.remove=TRUE, verbose=TRUE) # , "center", "scale"
...
set.seed(seed_value)
rows_shuffled <- sample(nrow(X))
X <- X[rows_shuffled, ]
Y <- Y[rows_shuffled]
ExpInd_final <- ExpInd_final[rows_shuffled]

# Initial computation of ICP without partitioning the dataset
icp_lasso <- ICP(data.matrix(X), Y, ExpInd_final, alpha = 0.05, selection = "lasso", showAcceptedSets = FALSE, showCompletion = FALSE)#"stability")#"all")#"boosting")#
print(icp_lasso)

icp_stability <- ICP(data.matrix(X), Y, ExpInd_final, alpha = 0.05, selection = "stability", showAcceptedSets = FALSE, showCompletion = FALSE)#"all")#"boosting")#"lasso")#
print(icp_stability)

# boosting was not considered
icp_boosting <- ICP(data.matrix(X), Y, ExpInd_final, alpha = 0.05, selection = "boosting", showAcceptedSets = FALSE, showCompletion = FALSE)#"lasso")#"stability")#"all")#
print(icp_boosting)

library(caret)

# Definition of summary for classification with MCC
multiClassSummaryOwnMCC <- function(data, lev = NULL, model = NULL){
  ...
  MCC <- (CM$table[2,2] * CM$table[1,1] - CM$table[2,1] * CM$table[1,2])/denom
...
# Compute CV of whole test just to get an initial reference (not used in the paper
cv_results <- list()
for (typeCausalityAnalysis in c("lasso", "stability", "boosting", "none")) {#
  ...
  if (typeCausalityAnalysis == "lasso") {
    ...
    trans_red <- cbind(Y, as.data.frame(X[, icp_lasso$usedvariables]))
    # trans_red <- cbind(Y_train, as.data.frame(X_selected))
    colnames(trans_red) <- c(LABEL_COL, colnames(X[, icp_lasso$usedvariables]))
  } else if (typeCausalityAnalysis == "stability") {
    computeBool <- !icp_stability$modelReject
    trans_red <- cbind(Y, as.data.frame(X[, icp_stability$usedvariables]))
    colnames(trans_red) <- c(LABEL_COL, colnames(X[, icp_stability$usedvariables]))
  } else if (typeCausalityAnalysis == "boosting") {
    computeBool <- !icp_boosting$modelReject
    trans_red <- cbind(Y, as.data.frame(X[, icp_boosting$usedvariables]))
    colnames(trans_red) <- c(LABEL_COL, colnames(X[, icp_boosting$usedvariables]))
  } else {
    computeBool <- TRUE
    correlationMatrix <- stats::cor(X)
    highlyCorrelated <- caret::findCorrelation(abs(correlationMatrix), cutoff=highlyCorrelatedCutoff)
    if (length(highlyCorrelated) != 0) {
      lowlyCorrelated <- colnames(X)[-highlyCorrelated]
      # lowlyCorrelated <- append(LABEL_COL, lowlyCorrelated)
    } else {
      lowlyCorrelated <- colnames(X)
    }

    trans_red <- cbind(Y, as.data.frame(X[,lowlyCorrelated]))
    colnames(trans_red) <- c(LABEL_COL, colnames(X[,lowlyCorrelated]))
  }
  if (computeBool) {
    ...
    # Radiomics + BIRADS
    p_radiomics_birads <- ggroc(list("RadiomicsCV" = roc_radiomics_birads_cv), aes = c("colour"), title = paste("ROC curve",typeCausalityAnalysis), legacy.axes = FALSE) +
    ...
    cv_radiomics_birads <- matrix(nrow = 8, ncol = 3, dimnames = list(c('AUC', 'Kappa', 'F1', 'Acc', 'Sens', 'Spec', 'PPV', 'NPV'), c('Mean', '95CI_Left', '95CI_Right')))
    print('CV Radiomics + BI-RADS')
...
# Partition data into training annd test sets
indx_train <- createDataPartition(Y, p = split_train, list = FALSE, times = 1)
...
ExpInd_train <- as.numeric(as.factor(ExpInd_final[indx_train]))

icp_lasso <- ICP(data.matrix(X_train), Y_train, ExpInd_train, alpha = 0.05, selection = "lasso", showAcceptedSets = FALSE, showCompletion = FALSE)#"stability")#"all")#"boosting")#
print(icp_lasso)

icp_stability <- ICP(data.matrix(X_train), Y_train, ExpInd_train, alpha = 0.05, selection = "stability", showAcceptedSets = FALSE, showCompletion = FALSE)#"all")#"boosting")#"lasso")#
print(icp_stability)

icp_boosting <- ICP(data.matrix(X_train), Y_train, ExpInd_train, alpha = 0.05, selection = "boosting", showAcceptedSets = FALSE, showCompletion = FALSE)#"lasso")#"stability")#"all")#
print(icp_boosting)

# Compute performance on training and test datasets
cv_results <- list()
coefs_model <- list()
test_results <- list()
for (typeCausalityAnalysis in c("lasso", "stability", "boosting", "none")) {#
  ...
  if (computeBool) {
    ...
    # Radiomics + BIRADS
    p_radiomics_birads <- ggroc(list("Radiomics + BIRADS CV" = roc_radiomics_birads_cv), aes = c("colour"), title = "ROC curve", legacy.axes = FALSE) +
      geom_ribbon(data=dat.ci_rb, aes(x = x, ymin = lower, ymax = upper), alpha = 0.3, inherit.aes = FALSE,fill = cols[1])+
      geom_line() + labs(x = "Specificity", y = "Sensitivity") + labs(colour = "Models") +
      annotate("text", x = .35, y = .5, color=cols[1], size = 6, label = paste("AUC =", round(roc_radiomics_birads_cv$auc, 2), ' [', round(ci_radiomics_birads_cv[1], 2), ',', round(ci_radiomics_birads_cv[3], 2), ']', sep = '')) +
      theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12),
